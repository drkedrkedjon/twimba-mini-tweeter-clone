...
document.addEventListener('click', function (e) {
  if (e.target.id === 'twitear-btn') {newTweet()}
  if (e.target.dataset.replies) {manejarRespuestas(e.target.dataset.replies)}
  if (e.target.dataset.likes) {manejarLikes(e.target.dataset.likes)}
  if (e.target.dataset.retweets) {manejarRetweets(e.target.dataset.retweets)} 
})
function manejarRetweets(tweetUuid) {
  const tweetEncontrado = twimbaFeed.find( tweet => tweet.uuid === tweetUuid)
  tweetEncontrado.isRetweeted ? tweetEncontrado.retweets-- : tweetEncontrado.retweets++
...
function manejarRespuestas(tweetUuid) {
  document.querySelector(`#tweet-replies-${tweetUuid}`).classList.toggle('ocultar-respuestas')
}
function newTweet() {
...
function generarHtml() {
  ...
  twimbaFeed.forEach(tweet => {
    ...
    let retweetColor = ''
    if (tweet.isRetweeted) {retweetColor = 'limegreen'}
    ...
    html += `
      <section class="tweet">
        ...
        <div class="tweet-contenido">
          ...
          <div class="ocultar-reply-bloque" id="reply-bloque">

          </div>

          <div class="ocultar-respuestas" id="tweet-replies-${tweet.uuid}">
            ${replyHtml}
          </div>
